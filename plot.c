...
void plot_web_positions(color col) {
    DrawCircleV(inp_pos, vertex_radius, col);
    DrawCircleV(out_pos, vertex_radius, col);
    for (i32 i = 0; i < V ; i++) {
        DrawCircleV(web_pos[i], vertex_radius, col);
    }
}
void plot_weights(color col) {
    ...
    for (i32 i = 0; i < H; i++) {
        source = inp_pos;
        target = web_pos[i];
        thickness = fabs(p[load_weight_index(i)]);
    ...
    for (i32 i = H; i < V; i++) {
        ...
        for (i32 j = 0; j < H; j++) {
            ...
            thickness = fabs(p[weight_index(s_from_t_p(i,j),i)])/scaling;
            if (thickness > 1.0) {
                DrawLineEx(target, source, thickness, col);
    ...
    for (i32 i = V - H; i < V; i++) {
        source = out_pos;
        target = web_pos[i];
        thickness = fabs(p[drain_weight_index(i)])/scaling;
        DrawLineEx(target, source, thickness, col);
    }
}